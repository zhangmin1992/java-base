 ...
 * 每个线程都有一个线程工作区，当线程访问一个对象值的时候，首先通过对象的引用找到对应在堆内存中的变量的值，然后把堆内存变量的具体值load到线程工作区，建立
 * 一个变量副本，之后线程就不在和堆内存中的对象值有任何关系，而是直接修改副本变量的值，在修改完线程退出之前，自动把线程变量副本的值写到堆内存中区。
 * volatile 关键字是因为能够保证每次读取前都能从主内存刷新最新值，每次写入后必须立即会写到主内存。
 * 指令重排序和在变量读写前插入内存屏障有关系，内存屏障的功能是:
 * 
 * 1.他确保指令重排序时不会将后面的执行排到内存屏障之前的位置，也不会把后面的指令排到内存屏障的后面，也就是说在执行到内存屏障的这句指令时，前面的操作已完成
